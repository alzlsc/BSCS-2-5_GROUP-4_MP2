...
public class optKnapsack {
    ...
    static class Item {
        int value, weight, origIndex;
        double score;
        

        Item(int value, int weight, int origIndex) {
            this.value = value;
            this.weight = weight;
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < bestChosen.length; i++) {
            if (bestChosen[i]) {
                System.out.println("Item " + (items[i].origIndex+1) + ": value = " + items[i].value + ", weight = " + items[i].weight);
            }
        }
    }
